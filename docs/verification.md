...
交付包：`C:\Users\34970\Desktop\Android\kebiao\Personal.Schedule-v1.2.0.apk`，`clean` 构建，993,817 字节，SHA256 `5BC7FD7CF03F3D0D83C185ECC42A5AA6C6ABAEE2D64FB50C354DE478F6E65913`。（此前两次刷新依次为 992,933 字节 / `43A017BA…` 和 993,829 字节 / `988FE6B3…`，以本行为准。）
...
**默认钟点**（第1–12节）换成使用者所在学校的课堂教学时间表（本人同意收录）：上午 08:30/09:20/10:25/11:15，下午 13:40/14:30/15:35/16:25，晚上 18:20/19:10/20:15/21:05。每节 45 分钟、课间 5 分钟、两段大课间 20 分钟。第13–16节仍是留白（22:00 起），因为默认值必须自身通过「保存作息」的重叠校验，而 22:00 之后不能再排 12 节以内的课。README 里那句"不代表任何学校的实际作息"随之改掉了——现在它确实是一所学校的真实作息。
...
模拟器实测（emulator-5554，API 37，用户真实数据，操作前先把 `kejian.db` 整份备份到 `C:\Users\34970\Desktop\Android\kebiao\kejian-backup-before-bell-check.db`）：
...
交付包已按本次改动重新 `clean` 构建并刷新到 `C:\Users\34970\Desktop\Android\kebiao\Personal.Schedule-v1.2.0.apk`（993,829 字节，SHA256 见上文交付包一行）。`tests\run-pure-java.cmd` 仍是 2120 + 14 + 30 项全通过，`python -m unittest discover -s tests` 5 项通过（本次没有改动被它们覆盖的代码）。Android Lint 本次由 19 条非阻断提示变为 **20 条**：多的一条是 `MainActivity.java` 里 `pickTime` 的 `target.setText(prefix + ...)`（`SetTextI18n`），与既有 9 条同类，属于本工程不使用字符串资源这一既有取舍，非新增错误；**错误数仍为 0**。

### 改动：设置页页脚写明使用范围，仓库补 LICENSE

起因：使用者担心被人拿去商用，自己把页脚从“Java 原生安卓 · 课表随身记”改成了“私人使用 切勿商用”（那行是使用者本人改的）。本次在此基础上把它补成完整的一句，并新增仓库级许可文件。

- `MainActivity.showSettings()` 页脚：`个人课表 1.2.0` / `私人使用，切勿商用 · 保留所有权利`。模拟器实测（覆盖安装）：该行在 640px 宽度内排成两行，`uiautomator dump` 的 bounds 为 `[40,950][680,1032]`，无截断（截图留在本机模拟器，按仓库惯例不随源码交付）。
...
**这一步只改了页脚一行 UI 文案**，逻辑与数据库未动；Lint 仍 0 错误 / 20 条非阻断提示。许可文件能说明作者的禁止意图，但**不能从技术上阻止**他人复制源码——公开仓库里的源码任何人都能取走，真正有效的手段是把仓库设为私有；这一点已当面告知使用者。
